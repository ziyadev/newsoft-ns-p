...
uses
  ...
  REST.Client, System.JSON, Vcl.WinXCtrls, Unit2, ShellAPI, System.Threading;
...
uses Unit4, DelphiZXIngQRCode;

procedure TAuthenticate.GenerateAuthenticationQRCode(Image: TImage);
...
begin
  ...
  try
    try
      ...
      if Assigned(Response) and Assigned(Response.Values['data']) and (Response.Values['code'].Value = 'SUCCESS') then
      begin
    ...
    for Row   := 0 to QRCode.Rows - 1 do
      for Col := 0 to QRCode.Columns - 1 do
        if QRCode.IsBlack[Row, Col] then
          Image.Picture.Bitmap.Canvas.FillRect(Rect(Col * CellSize, Row * CellSize, (Col + 1) * CellSize, (Row + 1) * CellSize));
...
function TAuthenticate.ValidateUserSession(SessionId: string): String;
var
  RESTClient: TRESTClient;
  RESTRequest: TRESTRequest;
  RESTResponse: TRESTResponse;
...
function TAuthenticate.AuthenticateAndGetUserDetails(Token: string): TUserDetails;
var
  RESTClient: TRESTClient;
  RESTRequest: TRESTRequest;
  RESTResponse: TRESTResponse;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        UserDetails := TUserDetails.Create(DataObject.Values['id'].Value, DataObject.Values['email'].Value, DataObject.Values['firstName'].Value, DataObject.Values['lastName'].Value, DataObject.Values['createdAt'].Value, // Same as joined at
          Token)
...
procedure TAuthenticate.ValidateSessionButtonClick(Sender: TObject);
var
  AuthToken: String;
begin
...
procedure TAuthenticate.VisitAuthButtonClick(Sender: TObject);
begin
  ShellExecute(0, 'OPEN', PChar(AuthUrl), nil, nil, SW_SHOWNORMAL);
  ValidateSessionButton.Enabled := True;
end;

procedure TAuthenticate.Button2Click(Sender: TObject);
begin
  case MessageDlg('Voulez vous Vraiment Quitter sans activer votre Copie?', mtWarning, [mbYes, mbNo], 0) of
    mrYes:
      begin
...
procedure TAuthenticate.FormCreate(Sender: TObject);
begin
  TTask.Run(
    procedure
    begin
